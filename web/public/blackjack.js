(function () {
  ...
  function render() {
    ...
    if (state.phase === 'playing') { var turn = state.players.find(function (p) { return p.id === state.turnPlayerId; }); message = myTurn ? '내 차례입니다. 히트 또는 스탠드를 선택하세요.' : (turn ? turn.nickname + '님의 카드 선택 차례' : '카드 선택 중'); }
    if (state.phase === 'betting') { var bettor = state.players.find(function (p) { return p.id === state.turnPlayerId; }); message = myTurn ? '내 차례입니다. 배팅 액션을 선택하세요.' : (bettor ? bettor.nickname + '님의 배팅 차례' : '배팅 중'); }
    ...
    $('players').innerHTML = state.players.map(function (player) { var status = player.isFolded ? '폴드' : player.isBusted ? '버스트' : player.isStanding ? '스탠드' : player.isAllIn ? '올인' : player.ready ? '준비' : '대기'; return '<div class="player ' + (player.id === state.turnPlayerId ? 'turn' : '') + '" data-id="' + player.id + '"><b>' + escapeHtml(player.nickname) + (player.id === state.you.id ? ' (나)' : '') + '</b><small>' + money(player.chips) + ' · 배팅 ' + money(player.roundBet) + '</small><span class="status">' + status + '</span></div>'; }).join('');
    $('cards').innerHTML = state.players.filter(function (player) { return player.cards.length; }).map(function (player) {
      ...
      return '<div class="seat blackjack-seat ' + (player.isFolded ? 'folded' : '') + '"><div class="hand">' + cards + '</div><b>' + escapeHtml(player.nickname) + '</b><span class="score ' + (player.isBusted ? 'bust' : '') + '">' + score + '</span></div>';
    }).join('');
    $('history').innerHTML = state.history.slice().reverse().map(function (item) { return '<div>' + escapeHtml(item.text) + '</div>'; }).join('');
    document.querySelectorAll('.player').forEach(function (element) { element.oncontextmenu = function (event) { event.preventDefault(); if (element.dataset.id === state.you.id) return; donationTarget = element.dataset.id; var target = state.players.find(function (p) { return p.id === donationTarget; }); $('donate-name').textContent = target.nickname + '님에게'; $('donate').style.left = Math.min(event.clientX, innerWidth - 190) + 'px'; $('donate').style.top = Math.min(event.clientY, innerHeight - 150) + 'px'; $('donate').classList.remove('hidden'); }; });
    renderProposal();
  }
  ...
  document.addEventListener('click', function (event) { if (!$('donate').contains(event.target)) $('donate').classList.add('hidden'); }); setInterval(function () { send('ping'); }, 20000);
}());
