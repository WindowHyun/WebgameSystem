(function () {
  ...
  function render() {
    ...
    document.querySelectorAll('.player').forEach(function (element) { element.oncontextmenu = function (event) { event.preventDefault(); if (state.phase !== 'lobby' && state.phase !== 'result') { showError('기부는 대기 중에만 할 수 있습니다.'); return; } if (element.dataset.id === state.you.id) return; donationTarget = element.dataset.id; var target = state.players.find(function (p) { return p.id === donationTarget; }); $('donate-name').textContent = target.nickname + '님에게'; $('donate').style.left = Math.min(event.clientX, innerWidth - 190) + 'px'; $('donate').style.top = Math.min(event.clientY, innerHeight - 150) + 'px'; $('donate').classList.remove('hidden'); }; });
    renderProposal();
  }
